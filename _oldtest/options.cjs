...
module.exports = function (Cron, test) {
  ...
  test(
    "* * * * * * with maxRuns: 1 should return null after 1.5 seconds",
  ...
  test(
    "* * * * * * with maxRuns: 1 should return null after 1.5 seconds (legacyMode)",
    timeout(2000, (resolve, reject) => {
      let scheduler = new Cron("* * * * * *", { maxRuns: 1, legacyMode: true });
      scheduler.schedule(function () {});
      setTimeout(function () {
        let nextRun = scheduler.nextRun();
        // Do comparison
  ...
  test(
    "* * * * * * with maxRuns: 1 should return null after 1.5 seconds (swapped argument order)",
    timeout(2000, (resolve, reject) => {
      let hasRun = false,
        scheduler = new Cron(
          "* * * * * *",
