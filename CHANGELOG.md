...
- **Date-based jobs scheduled in the past are now strict by default**: one-off jobs more than 1
  second in the past no longer auto-fire unless `allowPast: true` is set.
...
- Improved DST overlap handling for high-frequency schedules to keep UTC progression monotonic and
  avoid skip/gap behavior during fall-back transitions.
...
- Corrected the `existance` typo in `pattern.ts` comments.
- Added regression coverage for negative-delay clamping and hardened once-job tests around catch-up
  behavior.
...
- `sloppyRanges` option to allow relaxed, backward-compatible non-standard range/stepping syntax
  (#327)
...
- Comprehensive edge case tests for year stepping, nth weekday, W modifier, Quartz mode, and
  boundary conditions (#329)

### Changed

- **BREAKING**: `?` character now acts as wildcard alias (same as `*`) per OCPS 1.4, instead of
  substituting current time values (#293)
